...
Alphenix is an AI-native task management platform — like Linear, but with AI agents as first-class citizens.
...
- **Entry points** (`cmd/`): `server` (HTTP API), `alphenix` (CLI — daemon, agent management, config), `migrate`
...
- **CLI** (`internal/cli/`): Shared helpers for the `alphenix` CLI — API client, config management, output formatting.
...
make cli ARGS="..."   # Run alphenix CLI (e.g. make cli ARGS="config")
make test             # Go tests
...
pnpm --filter @alphenix/web exec vitest run src/path/to/file.test.ts
