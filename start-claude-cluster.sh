...
pkill -f "alphenix daemon" 2>/dev/null || true
sleep 2
...
for i in $(seq 1 $COUNT); do
    PORT=$((BASE_PORT + i - 1))
    LOG_FILE="$HOME/.alphenix/daemon-claude-$i.log"
    ...
    export ALPHENIX_DAEMON_ID="claude-$i"
    export ALPHENIX_HEALTH_PORT="$PORT"
    exec /opt/homebrew/bin/alphenix daemon start --foreground) > "$LOG_FILE" 2>&1 &
...
ps aux | grep "alphenix daemon" | grep -v grep | awk '{print "  PID:"$2, "Port:"19513+NR-1}'

echo ""
echo "=== Runtime Status ==="
TOKEN=$(cat ~/.alphenix/config.json | jq -r '.token')
WS_ID=$(cat ~/.alphenix/config.json | jq -r '.workspace_id')
...
echo ""
echo "Log files: ~/.alphenix/daemon-claude-*.log"
