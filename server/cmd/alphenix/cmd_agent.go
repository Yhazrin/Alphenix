...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, ALPHENIX_SERVER_URL env, or 'alphenix config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("ALPHENIX_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("ALPHENIX_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "ALPHENIX_SERVER_URL", "")
	if val != "" {
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "ALPHENIX_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
	profile := resolveProfile(cmd)
