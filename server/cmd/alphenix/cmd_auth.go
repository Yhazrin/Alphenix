...
var authLoginCmd = &cobra.Command{
	Use:    "login",
	Short:  "Authenticate with Alphenix",
	Long:   "Authenticate with Alphenix without auto-configuring workspaces. Use 'alphenix login' for the guided setup flow.",
	Hidden: true,
	RunE:   runAuthLogin,
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("ALPHENIX_TOKEN")); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"ALPHENIX_APP_URL", "FRONTEND_ORIGIN"} {
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'alphenix login' to authenticate.")
		return nil
	}
...
<title>Alphenix — Authenticated</title>
<style>
