...
// FetchLatestRelease fetches the latest release tag from the alphenix GitHub repo.
func FetchLatestRelease() (*GitHubRelease, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/multica-ai/alphenix/releases/latest", nil)
	if err != nil {
		return nil, err
	}
...
// IsBrewInstall checks whether the running alphenix binary was installed via Homebrew.
...
// UpdateViaBrew runs `brew upgrade alphenix/tap/alphenix`.
// Returns the combined output and any error.
func UpdateViaBrew() (string, error) {
	cmd := exec.Command("brew", "upgrade", "alphenix/tap/alphenix")
...
func UpdateViaDownload(targetVersion string) (string, error) {
	...
	// Build download URL: alphenix_{os}_{arch}.tar.gz
	tag := targetVersion
	if !strings.HasPrefix(tag, "v") {
		tag = "v" + tag
	}
	assetName := fmt.Sprintf("alphenix_%s_%s.tar.gz", runtime.GOOS, runtime.GOARCH)
	downloadURL := fmt.Sprintf("https://github.com/multica-ai/alphenix/releases/download/%s/%s", tag, assetName)
	...
	// Extract the "alphenix" binary from the tarball.
	binaryData, err := extractBinaryFromTarGz(resp.Body, "alphenix")
	if err != nil {
	...
	tmpFile, err := os.CreateTemp(dir, "alphenix-update-*")
	if err != nil {
